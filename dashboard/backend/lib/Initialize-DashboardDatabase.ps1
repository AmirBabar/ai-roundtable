...
Write-Host "📊 Initializing Council Dashboard Database..." -ForegroundColor Cyan
...
"@
    ...
    Write-Host "[OK] SQL schema saved to: $sqlScriptPath" -ForegroundColor Green
    Write-Host ""
    ...
    if ($sqliteExe) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "[WARN] Database initialization failed. Run the SQL command manually." -ForegroundColor Yellow
        }
    } else {
        Write-Host "[WARN] sqlite3 not found in PATH. Using Python sqlite3..." -ForegroundColor Yellow
...
"@
    ...
    Write-Host "  • api_calls      - Track all Council model API calls" -ForegroundColor Gray
    Write-Host "  • gateway_health  - LiteLLM gateway status monitoring" -ForegroundColor Gray
    Write-Host "  • model_stats     - Daily aggregated statistics" -ForegroundColor Gray
    Write-Host "  • budget_alerts   - Cost threshold alerts" -ForegroundColor Gray
    Write-Host "  • settings        - Dashboard configuration" -ForegroundColor Gray
    Write-Host "  • v_today_summary  - Today's statistics view" -ForegroundColor Gray
    Write-Host "  • v_gateway_status - Current gateway status view" -ForegroundColor Gray
    Write-Host ""
}
catch {
